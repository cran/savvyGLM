...
-   Replace the OLS update with one of several shrinkage estimators (e.g., `St_ost`, `DSh_ost`, `SR_ost`, `GSR_ost`, `LW_ost`, `QIS_ost`, and optionally `Sh_ost`).
...
1.  **Custom Optimization Methods:** Implements shrinkage estimators (`St_ost`, `DSh_ost`, `SR_ost`, `GSR_ost`, `LW_ost`, `QIS_ost`, and optionally `Sh_ost`) that replace the standard OLS update in IRLS.

2.  **Flexible Model Choice:** The model_class argument allows users to choose one or more shrinkage methods. By default, only `"St"`, `"DSh"`, `"SR"`, `"GSR"`, `"LW"`, and `"QIS"` are evaluated. Including `"Sh"` is optional because solving the Sylvester equation required by `Sh_ost` is computationally intensive.
...
| `model_class` | A character vector specifying the shrinkage model(s) to be used. Allowed values are `"St"`, `"DSh"`, `"SR"`, `"GSR"`, `"LW"`, and `"QIS"` and `"Sh"`. If multiple values are given, they are run in parallel and the best is chosen by AIC. If `"Sh"` is not included, it is omitted from the evaluation. | `c("St", "DSh", "SR", "GSR", "LW", "QIS")` |
...
| `use_robust_start` | Logical. [If `TRUE`, uses an optimization-based approach (via the CVXR package) to calculate robust starting values for fragile link functions (]{style="color:blue"}e.g. "sqrt"[).]{style="color:blue"} | `FALSE` |
